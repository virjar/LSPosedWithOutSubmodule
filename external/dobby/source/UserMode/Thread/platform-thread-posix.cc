...
OSThread::LocalStorageKey OSThread::CreateThreadLocalKey() {
  pthread_key_t key;
  [[maybe_unused]] int result = pthread_key_create(&key, nullptr);
  DCHECK_EQ(0, result);
...
void OSThread::DeleteThreadLocalKey(LocalStorageKey key) {
  ...
  [[maybe_unused]] int result = pthread_key_delete(pthread_key);
  DCHECK_EQ(0, result);
}
...
void OSThread::SetThreadLocal(LocalStorageKey key, void *value) {
  pthread_key_t pthread_key = LocalKeyToPthreadKey(key);
  [[maybe_unused]] int result = pthread_setspecific(pthread_key, value);
  DCHECK_EQ(0, result);
}
