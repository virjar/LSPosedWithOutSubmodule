...
std::vector<MemoryRegion> ProcessRuntimeUtility::GetProcessMemoryLayout() {
  ...
  std::qsort(&ProcessMemoryLayout[0], ProcessMemoryLayout.size(), sizeof(MemoryRegion),
             +[](const void* a, const void* b) -> int {
                   const auto *i = static_cast<const MemoryRegion *>(a);
                   const auto *j = static_cast<const MemoryRegion *>(b);
                   if ((addr_t)i->address < (addr_t)j->address) return -1;
                   if ((addr_t)i->address > (addr_t)j->address) return 1;
                   return 0;
             });
